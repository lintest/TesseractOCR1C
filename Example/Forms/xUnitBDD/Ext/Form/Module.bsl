...
Функция XML() Экспорт
	...
	Если ТипЗнч(ПроверяемоеЗначение) <> Тип("Строка") Тогда
		Возврат ПрерватьТест(ПроверяемоеЗначение, "Проверяемое значение не является строкой XML");
	КонецЕсли;
	
	Попытка
		ЧтениеXML = Новый ЧтениеXML();
		ЧтениеXML.УстановитьСтроку(ПроверяемоеЗначение);
		ПроверяемоеЗначение = ФабрикаXDTO.ПрочитатьXML(ЧтениеXML);
	Исключение
		Возврат ПрерватьТест(ПроверяемоеЗначение, "Не удалось получить прочитать XML");
	КонецПопытки;
